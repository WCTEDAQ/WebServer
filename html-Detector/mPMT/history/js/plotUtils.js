...
import {fetchMonitoringJSON, mapDataToTraces} from './fetchData.js';
...
function setPlotData(plotConfig, xdata, ydata) {
   return plotConfig.keys.map(key => {
      if (ydata.has(key)) {
         ...
         x_data_to_plot = xdata;
         y_data_to_plot = ydata.get(key);

         if ("differentiate" in plotConfig && plotConfig.differentiate === true) {
            // DIFFERENTIATION OF THE UPCOMING DATA
            const TIME_BETWEEN_THE_PACKETS_IN_SEC = 5;

            let diffNewData = differentiate(ydata.get(key), TIME_BETWEEN_THE_PACKETS_IN_SEC);
            
            x_data_to_plot = xdata.slice(1);
...
export async function extendPlots(plotConfigs) {
   ...
   const data = await fetchMonitoringJSON(timeOption);
