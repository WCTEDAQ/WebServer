...
var plots=["primary","level2","level1","t0_counts","tx_counts","mu_lead","hodo_0","hodo_1","tof_0","tof_1","hodo_display"];
//var plots=["primary","level2","level1","t0_counts","tx_counts","mu_lead","hodo_0","hodo_1","tof_0","tof_1"];
//var plots=["hodo_display"];
...
function Init(){
	...
	plotTraces.set('hodo_display',['HD-0','HD-8','HD-1','HD-9','HD-2','HD-A','HD-3','HD-B','HD-4','HD-C','HD-5','HD-D','HD-6','HD-E','HD-7']);
	...
	plotNames.set('hodo_display','Hodoscope Counts');
...
function makeplots(){
	...
	for(const plotname of plots){
		...
		else if(plotTypes.get(plotname)==='ED') makeeventdisplay(plotname);
	}
...
function updateplots(){
	...
	for(const plotname of plots){
		...
		else if(plotTypes.get(plotname)==='ED') updateeventdisplay(plotname);
	}
	const endTime = performance.now();
        console.log(`Call to update plots took ${endTime - startTime} milliseconds`);
	return;
}
...
async function updateeventdisplay(plotname){
	...
	try {
		...
		for(const trace of traces){
			...
			var command = `select time,data->'${trace}' as val from monitoring where device='BeamCounters' AND time >= now() - INTERVAL '3 days' order by time desc LIMIT ${numrows}`;
			...
			//console.log(trace+' '+result.at(0).val);

			y[iter] = result.at(0).val;
			if(iter<1.) lastupdates.set(plotname, result.at(0).time);
		        iter=iter+1;	
		}
		data.at(0).x = traces;
		data.at(0).y = y;

		let layout = {title: plotNames.get(plotname)};
		//console.log("Update event display");
...
async function makeeventdisplay(plotname){
	...
	const traces = plotTraces.get(plotname);
        let numrows = 1;
	let y = [];
        console.log('Make Event Display');
	try {
		let iter = 0.;
		for(const trace of traces){
			//var command = `select time,data->'${trace}' as val from monitoring where device='BeamCounters' order by time desc LIMIT ${numrows}`;
		...
		let data1 = dataMap.get(plotname);
        	console.log(data1.at(0).x);
        	console.log(data1.at(0).y);
        	console.log(data1.at(0).type);
